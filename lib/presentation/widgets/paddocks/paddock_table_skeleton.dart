...
class PaddockTableSkeleton extends StatelessWidget {
  final int rowCount;

  const PaddockTableSkeleton({super.key, this.rowCount = 10});
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: DataTable(
          ...
          // Las columnas deben ser IDÉNTICAS a las de PaddockTable para evitar saltos visuales
          columns: [
            const DataColumn(label: _HeaderText('NOMBRE')),
            const DataColumn(label: _HeaderText('UBICACIÓN')),
            if (MediaQuery.sizeOf(context).width > 1700)
              const DataColumn(label: _HeaderText('SUPERFICIE (mt2)')),
            if (MediaQuery.sizeOf(context).width > Responsive.mobileWidth)
              const DataColumn(label: _HeaderText('DESCRIPCIÓN')),
            if (MediaQuery.sizeOf(context).width > Responsive.tabletWidth)
              const DataColumn(label: _HeaderText('TIPO SUELO')),
          ...
          rows: List.generate(
            ...
            (index) => DataRow(
              cells: [
                _buildSkeletonCell(width: 40), // Nombre
                _buildSkeletonCell(width: 120), // ubicacion
                if (MediaQuery.sizeOf(context).width > 1700)
                  _buildSkeletonCell(width: 100), // superficie
                  if (MediaQuery.sizeOf(context).width > Responsive.mobileWidth)
                _buildSkeletonCell(width: 120), // descripcion
                if (MediaQuery.sizeOf(context).width > Responsive.tabletWidth)
                  _buildSkeletonCell(width: 100), //tipo suelo
