...
class SupplyTableSkeleton extends StatelessWidget {
  final int rowCount;

  const SupplyTableSkeleton({
    super.key,
    this.rowCount = 10,
  });
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: DataTable(
          ...
          // Las columnas deben ser IDÉNTICAS a las de SupplyTable para evitar saltos visuales
          columns: [
            const DataColumn(label: _HeaderText('NOMBRE')),
            const DataColumn(label: _HeaderText('TIPO')),
            if (MediaQuery.sizeOf(context).width > Responsive.tabletWidth)
              const DataColumn(label: _HeaderText('FECHA EXPIRACIÓN')),
