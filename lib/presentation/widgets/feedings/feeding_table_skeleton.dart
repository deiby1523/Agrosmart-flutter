    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: DataTable(
          ...
          // Las columnas deben ser IDÉNTICAS a las de FeedingTable para evitar saltos visuales
          columns: [
            const DataColumn(label: _HeaderText('FECHA INICIO')),
            const DataColumn(label: _HeaderText('FECHA FIN')),
            if (MediaQuery.sizeOf(context).width > 1300)
              const DataColumn(label: _HeaderText('LOTE')),
            const DataColumn(label: _HeaderText('CANTIDAD')),
            const DataColumn(label: _HeaderText('FRECUENCIA')),
            if (MediaQuery.sizeOf(context).width > 1000)
              const DataColumn(label: _HeaderText('INSUMO')),
          ...
          rows: List.generate(
            ...
            (index) => DataRow(
              cells: [
                _buildSkeletonCell(width: 50),
                _buildSkeletonCell(width: 120),
                if (MediaQuery.sizeOf(context).width > 1300)
                  _buildSkeletonCell(width: 40),
                _buildSkeletonCell(width: 100),
                _buildSkeletonCell(width: 80),
                if (MediaQuery.sizeOf(context).width > 1000)
                  _buildSkeletonCell(width: 100),
                _buildSkeletonCell(width: 80),
              ],
