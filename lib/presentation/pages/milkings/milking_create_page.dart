...
class MilkingCreatePage extends ConsumerStatefulWidget {
  final Milking? milking;

  const MilkingCreatePage({super.key, this.milking});

  @override
  ConsumerState<MilkingCreatePage> createState() => MilkingCreatePageState();
}

class MilkingCreatePageState extends ConsumerState<MilkingCreatePage> {
  ...
  void _initializeForm() {
    if (widget.milking != null) {
      _milkQuantityController.text = widget.milking!.milkQuantity.toString();
      _dateController.text = _formatDateForField(widget.milking!.date);
      _selectedLot = widget.milking!.lot;
    } else {
      // Establecer fecha actual por defecto
      final now = DateTime.now();
      _dateController.text = _formatDateForField(now);
    }
  }
  ...
  Widget _buildHeader(BuildContext context) {
    final isEditing = widget.milking != null;
    ...
    return Row(
      children: [
        ...
        const SizedBox(width: 12),
        Text(
          isEditing ? 'Editar Ordeño' : 'Nuevo Ordeño',
          ...
          Expanded(
            child: ElevatedButton(
              ...
              child: _isLoading
                  ...
                  : Text(isEditing ? 'Actualizar Ordeño' : 'Crear Ordeño'),
            ),
          ),
        ],
      ),
  ...
  List<DropdownMenuItem<Lot>> _buildLotList(
    ...
    if (_lots.isEmpty) {
      ...
      lotOptions = _lots.map((lot) {
        return DropdownMenuItem<Lot>(value: lot, child: Text(lot.name));
      }).toList();
    }
  ...
  Future<void> _submitForm() async {
    ...
    try {
      milkingDate = parseDate(_dateController.text.trim());
    } catch (e) {
    ...
    try {
      ...
      if (mounted) {
        context.go('/milkings');
        context.showSuccessSnack('Ordeño creado correctamente');
      }
    } catch (error) {
      if (mounted) {
